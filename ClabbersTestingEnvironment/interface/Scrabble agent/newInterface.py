...
class Game:
	...
	def showallgameover(self):
		...
		for player in [player1,player2]:
			...
			print('Score ' + str(player.rack), end = '')
			if player.score > otherplayer.score:
				print(' WINS')
			elif player.score < otherplayer.score:
				print(' LOSES')
			else:
				print('TIES')
			otherplayer = player1
	...
	def fullmove(self, showboard=False):
		...
		if ntilestodraw == 0 and len(self.players[self.activeplayer].rack)==0:
			# game over
			self.gameover=True
			rackbonus = 2 * sum([tilepoints[x] for x in self.players[3 - self.activeplayer].rack])
