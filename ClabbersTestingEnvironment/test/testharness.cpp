...
//	const LetterString letterString("ADIJEKL");
//	String rack(m_rack);
	game->currentPosition().setCurrentPlayerRack(Rack(QUACKLE_ALPHABET_PARAMETERS->encode(MARK_UV(m_rack.toStdString()))), true);
	if (game)
	{
...
void TestHarness::testPosition(const Quackle::GamePosition &position, Quackle::ComputerPlayer *player)
{
	...
	UVcout<<"unseen bag: "<<unseenBag.toString()<<endl;
